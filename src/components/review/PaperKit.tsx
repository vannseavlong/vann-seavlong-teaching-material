...
/** Blank ruled working space + answer line, with a "Check answer" button that jumps to `answerId` in Part B. */
  ...
  return (
    <div className="ml-10 mb-2">
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        <a
          href={`#${answerId}`}
          className="flex-shrink-0 inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full bg-purple-700 text-white hover:bg-purple-800 transition-colors whitespace-nowrap"
        >
          Check answer ↓
        </a>
      </div>
...
/** Answer-side sub-part: anchor target for the matching WorkSpace's "Check answer" button. Use letter="•" for flat questions. */
  ...
  return (
    <div id={id} className="mb-8 scroll-mt-28">
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 whitespace-nowrap">
          [{marks}]
        </span>
      </div>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                ...
                <td className="px-4 py-2.5 border border-slate-200 font-mono text-purple-700 text-sm">{row[2]}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="sticky top-16 z-30 bg-white/95 backdrop-blur-sm border-b border-slate-200 shadow-sm">
      <div className="max-w-4xl mx-auto px-6 py-2 flex gap-2 overflow-x-auto">
        {[
          { id: "exercise", label: "Part A — Exercise" },
          { id: "answers", label: "Part B — Answer Sheet" },
